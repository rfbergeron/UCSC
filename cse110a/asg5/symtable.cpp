      ...
      if (symval->attributes.test (i)) {
         if (i == (size_t) attr::STRUCT) {
            if (symval->fields == nullptr) {
               ...
               DEBUGH ('s',
                       "Printing " << static_cast<attr> (i) << " type");
               out << " " << static_cast<attr> (i) << " "
                   << *(symval->type_id);
            }
         } else {
...
symbol_value::symbol_value (astree* tree,
                            size_t sequence_,
                            size_t block_nr_):
...
vector<symbol_table*> type_checker::get_tables () { return tables; }
...
vector<symbol_entry> type_checker::sort_symtable (symbol_table* table) {
   ...
   while (sorted.size () < table->size ()) {
      ...
      for (auto& entry : *table) {
         ...
         if (entry.second->lloc.filenr <
             min_lloc->second->lloc.filenr) {
            min_lloc = &entry;
         } else if (entry.second->lloc.filenr ==
                          min_lloc->second->lloc.filenr &&
                    entry.second->lloc.linenr <
                          min_lloc->second->lloc.linenr) {
            min_lloc = &entry;
         } else if (entry.second->lloc.filenr ==
                          min_lloc->second->lloc.filenr &&
                    entry.second->lloc.linenr ==
                          min_lloc->second->lloc.linenr &&
                    entry.second->lloc.offset <=
                          min_lloc->second->lloc.offset) {
            min_lloc = &entry;
         }
      }
...
int type_checker::make_symbol_table (astree* root) {
   TYPE_ATTR_MASK.set ((size_t) attr::INT)
         .set ((size_t) attr::VOID)
         .set ((size_t) attr::STRING)
         .set ((size_t) attr::NULLPTR_T)
         .set ((size_t) attr::STRUCT)
         .set ((size_t) attr::ARRAY);
      ...
      switch (child->symbol) {
         ...
         case '=':
            ...
            if (!types_compatible (child->first (), child->second ())) {
               cerr << "ERROR: Incompatible types for global" << endl;
               return -1;
            ...
            child->attributes |=
                  (child->second ()->attributes & TYPE_ATTR_MASK);
            if (status != 0) return status;
            break;
         default:
...
int type_checker::make_global_entry (astree* global) {
   if (globals->find (extract_ident (global)) != globals->end ()) {
      ...
      symbol_value* global_value = new struct symbol_value (identifier);
   ...
   if (type_names->find (structure_type) != type_names->end ()) {
      ...
      for (size_t i = 1; i < structure->children.size (); ++i) {
         ...
         if (fields->find (field_id_str) != fields->end ()) {
            ...
            symbol_value* field_value =
                  new struct symbol_value (field->second (), i - 1);
            fields->insert ({field->second ()->lexinfo, field_value});
            DEBUGH (
                  't',
                  "Field inserted at " << *(field->second ()->lexinfo));
         }
      }
...
int type_checker::make_function_entry (astree* function) {
   ...
   for (size_t i = 0; i < params->children.size (); ++i) {
      ...
      if (status != 0) return status;
      struct symbol_value* param_entry =
            new struct symbol_value (param->second (), i, next_block);
      locals->insert ({param_id_str, param_entry});
      function_entry->parameters.push_back (param_entry);
   }

   DEBUGH ('t',
           "Inserting function entry with block id " << next_block);
   if (globals->find (function_id) != globals->end ()) {
      ...
      if (not functions_equal (prototype, function_entry)) {
         cerr << "ERROR: function has already been defined with a "
              << "different return type/parameters: " << function_id
              << endl;
         return -1;
         ...
         DEBUGH ('t',
                 "Completing entry for prototype " << *(function_id)
                                                   << endl);
         size_t sequence_nr = 0;
         status = validate_block (function->third (),
                                  function_id,
                                  sequence_nr);
         if (status != 0) return status;
      ...
      if (function->children.size () == 3) {
         DEBUGH ('t',
                 "No protoype; defining function " << *(function_id)
                                                   << endl);
         size_t sequence_nr = 0;
...
int type_checker::make_local_entry (astree* local,
   ...
   if (locals->find (extract_ident (local)) != locals->end ()) {
      ...
      DEBUGH ('t',
              "Making entry for local var " << identifier->lexinfo);
      identifier->attributes.set ((size_t) attr::LVAL);
      identifier->attributes.set ((size_t) attr::VARIABLE);
      ...
      symbol_value* local_value =
            new struct symbol_value (local->second (),
                                     sequence_nr,
                                     next_block);
...
int type_checker::validate_block (astree* block,
   ...
   for (astree* statement : block->children) {
      int status =
            validate_stmt_expr (statement, function_name, sequence_nr);
      if (status != 0) return status;
   }
   return 0;
}
...
int type_checker::validate_stmt_expr (astree* statement,
   ...
   switch (statement->symbol) {
      ...
      case TOK_BLOCK:
         status =
               validate_block (statement, function_name, sequence_nr);
         if (status != 0) return status;
         break;
...
int type_checker::validate_type_id (astree* type, astree* identifier) {
   DEBUGH ('t',
           "Validating typeid of symbol " << *(identifier->lexinfo));
      ...
      case TOK_VOID:
         ...
         if (identifier->attributes.test ((size_t) attr::ARRAY)) {
            cerr << "ERROR: you may not have arrays of type void!"
                 << endl;
            return -1;
         } else if (!identifier->attributes.test (
                          (size_t) attr::FUNCTION)) {
...
int type_checker::validate_call (astree* call) {
   ...
   if (globals->find (identifier) != globals->end ()) {
      ...
      cerr << "ERROR: Invalid call to function: " << *identifier
           << endl;
      return -1;
   }
}
...
int type_checker::assign_type (astree* ident) {
   ...
   if (locals->find (id_str) != locals->end ()) {
      ...
      cerr << "ERROR: could not resolve symbol: " << (ident->lexinfo)
...
bool type_checker::functions_equal (symbol_value* f1,
                                    symbol_value* f2) {
...
bool type_checker::types_compatible (astree* tree,
                                     symbol_value* entry) {
...
void type_checker::set_block_nr (astree* tree, size_t nr) {
   tree->block_nr = nr;
   for (auto iter = tree->children.begin ();
...
void type_checker::dump_symbols (ostream& out) {
   ...
   for (auto&& itor = sorted_structures.begin ();
      ...
      for (auto&& itor2 = sorted_fields.begin ();
           itor2 != sorted_fields.end ();
           ++itor2) {
         out << "   " << *(itor2->first) << " " << itor2->second
             << endl;
      }
      out << endl;
   }
...
void type_checker::destroy_tables () {
   ...
   for (auto local_iter = tables.begin (); local_iter != tables.end ();
        ++local_iter) {
