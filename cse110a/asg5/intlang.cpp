...
void generator::set_out (ostream* out_) { out = out_; }
...
int generator::write_int_lang (astree* root,
   ...
   *out << endl;
   ...
   for (auto&& itor = statements.cbegin (); itor != statements.cend ();
        ++itor) {
...
int generator::write_function_decl (astree* fun, symbol_table* locals) {
   ...
   *out << ")" << endl;
   ...
   for (auto&& itor = sorted_locals.cbegin ();
        itor != sorted_locals.cend ();
        ++itor) {
      write_var_decl (*itor);
   }
...
string generator::write_stmt_expr (astree* expr,
                                   const string*& label,
   ...
   switch (expr->symbol) {
      ...
      case '=':
         ...
         destination =
               write_stmt_expr (expr->first (), label, do_compound);
         source = write_stmt_expr (expr->second (),
                                   label,
                                   not do_compound);
      ...
      case TOK_CALL:
         ...
         for (size_t i = 1; i < expr->children.size (); ++i) {
            param_strings.push_back (
                  write_stmt_expr (expr->children[i], label));
         }
      ...
      case TOK_WHILE:
         ...
         WRLABEL (*label,
                  "goto " << branch_label.substr (
                        0,
                        branch_label.length () - 1));
         *out << skip_label << endl;
         return "";
      case TOK_IF:
         ...
         if (expr->children.size () == 3) {
            WRLABEL (*label,
                     "goto " << else_label.substr (
                                      0,
                                      else_label.length () - 1)
                             << " if ! " << left_operand);
         } else {
            WRLABEL (*label,
                     "goto " << skip_label.substr (
                                      0,
                                      skip_label.length () - 1)
                             << " if ! " << left_operand);
         }
...
string generator::write_reg_type (astree* tree) {
   if (tree->has_attr (attr::ARRAY) || tree->has_attr (attr::STRUCT) ||
