...
int type_checker::validate_stmt_expr (astree* statement,
   ...
   switch (statement->symbol) {
      ...
      case TOK_INDEX:
         ...
         if (!(statement->first ()->attributes.test (
                     (size_t) attr::ARRAY) &&
               statement->second ()->attributes.test (
                     (size_t) attr::INT))) {
...
void type_checker::dump_symbols (ostream& out) {
   ...
   for (auto iter = type_names->begin (); iter != type_names->end ();
      ...
      for (auto fiter = iter->second->fields->begin ();
           fiter != iter->second->fields->end ();
           ++fiter) {
...
void type_checker::destroy_tables () {
   ...
   for (auto local_iter = local_tables.begin ();
        local_iter != local_tables.end ();
        ++local_iter) {
