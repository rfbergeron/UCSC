...
;
structdef
: TOK_STRUCT TOK_IDENT '{' structbody '}' ';'                   { $$ = $1->adopt($2, $4); } 
| TOK_STRUCT TOK_IDENT '{' '}' ';'                              { $$ = $1->adopt($2); }
;
structbody
: type TOK_IDENT ';'                                            { $$ = $1->adopt($2); }
| structbody type TOK_IDENT ';'                                 { $$ = $1->buddy_up($2->adopt($3)); }
;
function
: type TOK_IDENT '(' parameters ')' block                       { $$ = parser::make_function($1, $2, $3, $4, $6); }
...
;
block
: '{' statements '}'                                            { $$ = $1->adopt_sym(TOK_BLOCK, $2);  }
...
;
vardecl
: type TOK_IDENT '=' expr ';'                                   { $$ = parser::make_type_id($1,$2, $4); }
| type TOK_IDENT ';'                                            { $$ = parser::make_type_id($1,$2); }
;
while
: TOK_WHILE '(' expr ')' statement                              { $$ = $1->adopt($3,$5); }
;
ifelse
: TOK_IF '(' expr ')' statement TOK_ELSE statement              { $$ = $1->adopt($3, $5,$7); }
