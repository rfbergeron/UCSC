...
stringOfList f [] = "[]"
stringOfList f xs = "[" ++ (sepConcat ", " ys) ++ "]"
  where
  	ys			  = [f x | x <- xs]
...
padZero l1 l2
    | length l1 < length l2 = (pad ++ l1, l2)
    | otherwise 			= (l1, pad ++ l2)
  where
  	pad
  	  | length l1 < length l2 = clone 0 (length l2 - length l1)
  	  | otherwise             = clone 0 (length l1 - length l2)
...
removeZero [] = []
removeZero (d:ds) = if d == 0 then removeZero ds else (d:ds)
...
bigAdd l1 l2     = removeZero (reverse (tail (fst (unzip res))))
  where
    (l1', l2')   = padZero l1 l2
    res          = foldLeft f base args
    f a x        = a ++ [(mod sum 10, quot sum 10)]
      where
      	sum = (fst x + snd x + snd (last a))
    base         = [(0,0)]
...
bigMul l1 l2 = res
  where
    ...
    f a x    = (mulByDigit 10 (fst a), bigAdd (snd a) (mulByDigit x (fst a)))
    base     = (l1, [])
    args     = reverse l2
