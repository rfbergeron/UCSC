...
eval env ENil = VNil
eval env (EVar target) = lookupId target env
...
evalOp Div (VInt a) (VInt b) = VInt (quot a b)
...
evalOp Eq (VPair a b) (VPair c d) = VBool (left && right)
  where
    (VBool left) = evalOp Eq a c
    (VBool right) = evalOp Eq b d
evalOp Eq _ _ = throw (Error "eq type error")
evalOp Ne (VInt a) (VInt b) = VBool (not (a == b))
evalOp Ne (VBool a) (VBool b) = VBool (not (a == b))
...
evalOp Cons (VInt a) (VPair b c) = VPair (VInt a) (VPair b c)
evalOp Cons (VInt a) (VNil) = VPair (VInt a) VNil
evalOp Cons (VPair a b) (VPair c d) = throw (Error "nope cant do that")
evalOp Cons _ _ = throw (Error "type error")
evalOp _ _ _ = throw (Error ("no matches")) 
...
--------------------------------------------------------------------------------
lookupId target [] = throw (Error ("unbound variable: " ++ target))
lookupId target (x:xs) 
  | target == fst x = snd x
  | otherwise = lookupId target xs
