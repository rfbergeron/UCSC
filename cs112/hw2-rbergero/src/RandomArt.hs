...
exprToString VarX                 = "x"
exprToString VarY                 = "y"
exprToString (Sine e)             = concat["sin(pi*", exprToString e, ")"]
exprToString (Cosine e)           = concat["cos(pi*", exprToString e, ")"]
exprToString (Average e1 e2)      = concat["(", exprToString e1, ") + (", exprToString e2, ")/2"]
exprToString (Times e1 e2)        = concat["(", exprToString e1, ") * (", exprToString e2, ")"]
exprToString (Thresh e1 e2 e3 e4) = concat[exprToString e1, "<", exprToString e2, " ? ", exprToString e3, " : ", exprToString e4]
...
build 0
  ...
  where
    r         = rand 10
build d       = deeper (rand 5)
  where
    deeper :: Int -> Expr
    deeper 4 = Sine (build (d - 1))
    deeper 3 = Cosine (build (d - 1))
    deeper 2 = Average (build (d - 1)) (build (d - 1))
    deeper 1 = Times (build (d - 1)) (build (d - 1))
    deeper n = Thresh (build (d - 1)) (build (d - 1)) (build (d - 1)) (build (d - 1))
--------------------------------------------------------------------------------
-- | Best Image "Seeds" --------------------------------------------------------
--------------------------------------------------------------------------------
