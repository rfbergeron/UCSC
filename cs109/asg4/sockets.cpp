...
void base_socket::create () {
   ...
   int on = 1;
   int status = ::setsockopt (socket_fd,
                              SOL_SOCKET,
                              SO_REUSEADDR,
                              &on,
                              sizeof on);
...
void base_socket::connect (const string host, const in_port_t port) {
   ...
   int status = ::connect (socket_fd,
                           ...
                           sizeof (socket_addr));
   if (status < 0)
      throw socket_sys_error ("connect(" + host + ":" +
                              to_string (port) + ")");
}
...
string to_string (const in_addr& ipv4_addr) {
   ...
   const char* result =
         ::inet_ntop (AF_INET, &ipv4_addr, buffer, sizeof buffer);
...
hostinfo::hostinfo (const in_addr& ipv4_addr):
      hostinfo (
            ::gethostbyaddr (&ipv4_addr, sizeof ipv4_addr, AF_INET)) {}
