...
void cix_get (client_socket& server, string filename) {
   ...
   if (header.command != cix_command::FILEOUT) {
      ...
      if ((outstream.rdstate () & ifstream::failbit) != 0) {
         cerr << "get " << header.filename << ": failed to write file"
              << endl;
         return;
      }
...
void cix_rm (client_socket& server, string filename) {
   ...
   snprintf (header.filename, sizeof header.filename, filename.data ());
   DEBUGF ('y', "sending header " << header);
   send_packet (server, &header, sizeof header);
...
void usage () {
   cerr << "Usage: " << logger.execname () << " [-@ flag] [host] [port]"
        << endl;
   throw cix_exit ();
}
