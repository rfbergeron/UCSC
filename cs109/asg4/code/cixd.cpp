...
void reply_rm (accepted_socket& client_sock, cix_header& header){
   ...
   if (status < 0) {
      cerr << "rm " << header.filename << ": unlink failed: "
         << strerror (errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet (client_sock, &header, sizeof header);
...
void reply_put (accepted_socket& client_sock, cix_header& header) {
   ...
   if ((outstream.rdstate() & ifstream::failbit) != 0){
      ...
      cerr << "put " << header.filename << ": failed to write file: "
         << strerror (errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet (client_sock, &header, sizeof header);
...
void reply_get(accepted_socket& client_sock, cix_header& header) {
   ...
   if ((instream.rdstate() & ifstream::failbit) != 0) {
      cerr << "get " << header.filename << ": failed to read file: "
         << strerror (errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet (client_sock, &header, sizeof header);
      ...
      for (;;) {
         ...
         switch (header.command) {
            ...
            case cix_command::RM:
               ...
               break;
            default:
               cerr << "invalid header from client:"
                  << header << endl;
               break;
         }
      }
...
int main (int argc, char** argv) {
   ...
   try {
      ...
      for (;;) {
         DEBUGF('y', to_string (hostinfo()) << " accepting port "
             << to_string (port));
         ...
         for (;;) {
            try {
               ...
               switch (error.sys_errno) {
                  case EINTR:
                     DEBUGF('y', "listener.accept caught "
                         << strerror (EINTR));
                     break;
                  default:
                     throw;
