...
shape::shape () { DEBUGF ('c', this); }

text::text (string bitmap_name, const string& textdata_):
      glut_bitmap_font (fontcode[bitmap_name]),
      textdata (textdata_) {
   DEBUGF ('c', this);
}
...
void text::draw_border (const vertex& center,
   ...
   DEBUGF (
         'd',
         this << "(" << center << "," << color << "," << width << ")");
   const GLubyte* glgreeting =
         reinterpret_cast<const GLubyte*> (textdata.c_str ());

   GLfloat greeting_width (
         glutBitmapLength (glut_bitmap_font, glgreeting));
