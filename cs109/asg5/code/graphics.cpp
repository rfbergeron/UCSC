...
mouse window::mus;
rgbcolor window::bordercolor (255, 0, 0);
GLfloat window::borderwidth (4);
GLfloat window::stepsize (4);
...
void window::display() {
   ...
   for(size_t i = 0 ; i < objects.size() ; i++) {
      if(i == selected_obj)
         objects[i].draw_border(bordercolor, borderwidth);
      objects[i].draw();
   }
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      case 'Q': case 'q': case ESC:
         ...
         break;
      case 'H': case 'h':
         move_selected_object (-stepsize, 0);
         break;
      case 'J': case 'j':
         move_selected_object (0, -stepsize);
         break;
      case 'K': case 'k':
         move_selected_object (0, +stepsize);
         break;
      ...
      case 'N': case 'n': case SPACE: case TAB:
         ++selected_obj;
         if(selected_obj >= objects.size()) selected_obj 
            -= objects.size();
         break;
...
void window::special (int key, int x, int y) {
   ...
   window::mus.set (x, y);
   switch (key) {
      case GLUT_KEY_LEFT: move_selected_object (-stepsize, 0); break;
      case GLUT_KEY_DOWN: move_selected_object (0, -stepsize); break;
      case GLUT_KEY_UP: move_selected_object (0, +stepsize); break;
      case GLUT_KEY_RIGHT: move_selected_object (+stepsize, 0); break;
...
void window::select_object(size_t to_select) {
   if(to_select >= objects.size())
      cerr << "cannot select object " << to_select <<
         ": too few objects" << endl;
   else
      selected_obj = to_select;
}
