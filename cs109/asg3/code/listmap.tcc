...
listmap<Key,Value,Less>::insert (const value_type& pair) {
   ...
   DEBUGF ('y', "Addresses that should match:\n" <<
...
listmap<Key,Value,Less>::erase (iterator position) {
   DEBUGF ('l', &*position);
   if(!empty()) {
      listmap<Key, Value, Less>::node * target_prev = position.where->prev;
      listmap<Key, Value, Less>::node * target_next = position.where->next;
