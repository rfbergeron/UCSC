...
ubigint ubigint::operator+ (const ubigint& that) const {
   ...
   udigit_t carry = 0;
   udigit_t digit;
   vector<udigit_t>::const_iterator that_iter = that.ubig_value.begin();
...
ubigint ubigint::operator- (const ubigint& that) const {
   ...
   udigit_t carry = 0;
   udigit_t digit;
   vector<udigit_t>::const_iterator iter_that = that.ubig_value.begin();
...
ubigint ubigint::operator* (const ubigint& that) const {
   ...
   for(unsigned int i = 0 ; i < ubig_value.size() ; ++i) {
      ...
      for(unsigned int j = 0 ; j < that.ubig_value.size() ; ++j) {
         udigit_t partial = product.at(i + j) + 
...
ostream& operator<< (ostream& out, const ubigint& that) {
   vector<udigit_t>::reverse_const_iterator rit = that.ubig_value.rbegin();
   string output;
